...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function atualizarCarrinho() {
        ...
        carrinho["produtos"].forEach(prod => {
            ...
            if (!produtosAgrupados[nome])
                produtosAgrupados[nome] = { produto: prod, quantidade: 0 };
        ...
        Object.values(produtosAgrupados).forEach(({ produto, quantidade }) => {
            ...
            btn.onclick = () => {
                for (let i = 0; i < quantidade; i++) {
                    carrinho.removerProduto(carrinho["produtos"].indexOf(prod));
                }
                atualizarCarrinho();
            };
            li.appendChild(img);
    ...
    document.querySelectorAll("[data-filter]").forEach(btn => {
        btn.addEventListener("click", () => { mostrarProdutos(btn.dataset.filter); });
    });
    // Cupom
    document.getElementById("aplicar-cupom").addEventListener("click", () => {
        ...
        if (cupom === "MENGAO2025") {
            cupomAtivo = true;
            descontoMsg.textContent = "Cupom aplicado! 10% de desconto.";
        }
        else {
            cupomAtivo = false;
    ...
    function atualizarMenuLogin() {
        ...
        `;
                (_a = document.getElementById("logout-link")) === null || _a === void 0 ? void 0 : _a.addEventListener("click", (e) => {
                    e.preventDefault();
